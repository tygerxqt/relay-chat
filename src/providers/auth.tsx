...
export const AuthContext = React.createContext<AuthSession>({
	...
	avatar: pb.authStore.model?.avatar
		? `https://relay.pockethost.io/api/files/_pb_users_auth_/${pb.authStore.model.id}/${pb.authStore.model.avatar}`
	...
	banner: pb.authStore.model?.banner
		? `https://relay.pockethost.io/api/files/_pb_users_auth_/${pb.authStore.model.id}/${pb.authStore.model.banner}`
	...
	async function uploadAvatar(file: File) {
		...
		toast.promise(
			pb.collection("users").update(pb.authStore.model?.id as string, formData),
			{
				...
				success: (data) => {
					setAvatar(
						`https://relay.pockethost.io/api/files/_pb_users_auth_/${data.id}/${data.avatar}`
					);
	...
	async function uploadBanner(file: File) {
		...
		toast.promise(
			pb.collection("users").update(pb.authStore.model?.id as string, formData),
			{
				...
				success: (data) => {
					setBanner(
						`https://relay.pockethost.io/api/files/_pb_users_auth_/${data.id}/${data.banner}`
					);
			...
			.then((response) => {
				...
				setAvatar(
					response.record?.avatar
						? `https://relay.pockethost.io/api/files/_pb_users_auth_/${response.record.id}/${response.record.avatar}`
						: `https://api.dicebear.com/7.x/lorelei-neutral/png?seed=${pb.authStore.model?.username}&radius=50`
				);
